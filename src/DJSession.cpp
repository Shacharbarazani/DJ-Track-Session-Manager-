...
int DJSession::load_track_to_controller(const std::string& track_name) {
    AudioTrack* t = library_service.findTrack(track_name);
    if (t == nullptr) {
    ...
    int res = controller_service.loadTrackToCache(*t);
    if (res == 1) {
        ++stats.cache_hits;
    }
    if (res == 0 ) {
...
bool DJSession::load_track_to_mixer_deck(const std::string& track_title) {
    ...
    AudioTrack* t = controller_service.getTrackFromCache(track_title);
    bool ret = true;
    if (t == nullptr) {
        ...
        int res = mixing_service.loadTrackToDeck(*t);
        if (res == 1) {
            ++stats.deck_loads_b;
            ++stats.transitions;
        }
...
void DJSession::simulate_dj_performance() {
    ...
    while (running) {
        ...
        if (play_all) {
            size_t n = session_config.playlists.size();
            std::string* arr = new std::string[n];
            size_t i = 0;
            for (const std::pair<const std::string, std::vector<int>>& p : session_config.playlists) {
                arr[i++] = p.first;
            }
            std::sort(arr, arr + n);
